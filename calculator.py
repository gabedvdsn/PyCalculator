...
class Token:
    ...
    def _inner_breakdown(self):
        ...
        if self.expr[self.start] == '(':
            left_expr = self._get_parentheses_field(self.start + 1)
        ...
        if any([left_expr.endswith(oper) for oper in self.calc.operations.keys()]):
            left_expr = self._get_fields_having(self.start, [])
    ...
    def _get_parentheses_field(self, start: int):
        i = start
        count = 0
        while i < len(self.expr):
            if self.expr[start + i] == ')':
                if count == 0:
                    return self.expr[start:start + i]
                count -= 1
            if self.expr[start + i] == '(':
                count += 1
